...
if(exists('solidity-ci.json')):
    data = load(open('solidity-ci.json'))
    ...
    if 'is_truffle_project' in data and isinstance(data['is_truffle_project'], bool):
        ...
        is_brownie_project = False
    if 'is_brownie_project' in data and isinstance(data['is_brownie_project'], bool):
        is_brownie_project = data['is_brownie_project']
    else:
        is_brownie_project = False
else:
    fail_on_warning = False
...
# Handling brownie projects
# Assuming we 'export MYTHX_API_KEY=YourToken' or set as an ENV 
if(is_brownie_project):
    log.write("Processing Brownie Project\n")
    result = check_output(['brownie analyze --mode=standard', 'reports/']).decode("utf-8")
    log.write(result)
    log.write("\nProcessing of Brownie Project Completed")
    if(re.search('Error', result, re.IGNORECASE)):
      log.write("\nFound Error in Solidity Code. Failing Build")
      exit(1)
